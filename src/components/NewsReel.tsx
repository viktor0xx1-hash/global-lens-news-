      ...
      <div className="flex">
        ...
        >
          {displayArticles.map((article, idx) => {
            const articleUrl = `/article/${article.category ? slugify(article.category) : 'intelligence'}/${article.id}/${slugify(article.title)}`;
            return (
              <Link 
                key={`${article.id}-${idx}`}
                to={articleUrl}
                className="flex-shrink-0 w-64 md:w-80 px-4 group border-r border-gray-200/50"
              >
